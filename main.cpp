...
void* memory_reallocator(void* pointer, size_t size)
{
    ...
    if (get_size(pointer) > size) // если блок имеет больше памяти чем ми хотим установить
    {
                                     ...
                                     >= size)
    {
        new_header((uint8_t*)pointer + size + SIZE_H, false, size, get_size(get_next(pointer)) -
                                                                   (size - get_size(pointer))); //создаём новый блок в наале первого и теперь первый находится между новым и вторым
        set_size(pointer, size);
        return pointer;
    }
    if (get_next(pointer) != nullptr && get_status(get_next(pointer)) == 0 &&
        get_size(pointer) + get_size(get_next(pointer)) + SIZE_H >= size) // если второй блок свободен, просто добавлем его во второй блок
    {
